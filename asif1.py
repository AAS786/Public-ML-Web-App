...
if (selected == 'Liver Prediction'):
    ...
    if st.button('**Liver Test Result**'):
        ...
        liver_prediction = liver_model.predict([[Age,Total_Bilirubin,Direct_Bilirubin,Alkaline_Phosphotase,
                                                 Alamine_Aminotransferase,Aspartate_Aminotransferase,Total_Proteins,Albumin,Albumin_and_Globulin_Ratio,Gender_Female,Gender_Male]])
...
if (selected == 'Kidney Prediction'):
    ...
    with col2:
        rbc = st.text_input('**Red Blood Cells**', ['normal', 'abnormal'])
        pc = st.text_input('**Pus Cell**', ['normal', 'abnormal'])
        pcc = st.text_input('**Pus Cell Clumps**', ['notpresent', 'present'])
        ba = st.text_input('**Bacteria**', ['notpresent', 'present'])
        pot = st.text_input('**Potassium**')
        wc = st.text_input('**White Blood Cell Count**')
    with col3:
        htn = st.text_input('**Hypertension**', ['yes', 'no'])
        dm = st.text_input('**Diabetes Mellitus**', ['yes', 'no'])
        cad = st.text_input('**Coronary Artery Disease**', ['yes', 'no'])
        pe = st.text_input('**Pedal Edema**', ['yes', 'no'])
        ane = st.text_input('**Anemia**', ['yes', 'no'])
