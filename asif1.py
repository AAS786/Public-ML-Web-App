...
if selected == 'Liver Prediction':
    #liver_img = "path_to_liver_image.jpg"  # Make sure to provide the correct path to the image
    ...
    with col2:
        Gender = st.selectbox('**Gender**', options=['Male', 'Female'])  # Use selectbox for gender input
    with col1:
    ...
    # Function to convert inputs to numeric values and handle errors
    def convert_to_numeric(value, name):
        try:
            return float(value)
        except ValueError:
    ...
    Age = convert_to_numeric(Age, "Age")
    # Convert Gender to numeric: Male = 1, Female = 0
    Gender = 1 if Gender == 'Male' else 0
    Total_Bilirubin = convert_to_numeric(Total_Bilirubin, "Total Bilirubin")
    Direct_Bilirubin = convert_to_numeric(Direct_Bilirubin, "Direct Bilirubin")
    Alkaline_Phosphotase = convert_to_numeric(Alkaline_Phosphotase, "Alkaline Phosphotase")
    Alamine = convert_to_numeric(Alamine, "Alamine Aminotransferase")
    Aspartate = convert_to_numeric(Aspartate, "Aspartate Aminotransferase")
    Total_Proteins = convert_to_numeric(Total_Proteins, "Total Proteins")
    Albumin = convert_to_numeric(Albumin, "Albumin")
    Albumin_Globulin_Ratio = convert_to_numeric(Albumin_Globulin_Ratio, "Albumin and Globulin Ratio")

    # Check if any conversion failed
    if None in [Age, Total_Bilirubin, Direct_Bilirubin, Alkaline_Phosphotase, Alamine, Aspartate, Total_Proteins, Albumin, Albumin_Globulin_Ratio]:
        st.stop()  # Stop execution if there is an invalid input

    # Prepare the input array
    input_data = np.array([[Age, Gender, Total_Bilirubin, Direct_Bilirubin, Alkaline_Phosphotase, Alamine, Aspartate, Total_Proteins, Albumin, Albumin_Globulin_Ratio]])
    ...
    if st.button('**Liver Test Result**'):
        ...
        if liver_prediction[0] == 1:
            liver_diagnosis = 'The person has a Liver Problem'
            result_color = "red"
        else:
            liver_diagnosis = 'The person does not have a Liver Problem'
            result_color = "green"
