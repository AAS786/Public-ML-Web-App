...
heart_model = pickle.load(open('C:/Users/dell/Downloads/heart_model.sav', 'rb'))

kidney_model = pickle.load(open('C:/Users/dell/Downloads/kidney_model.sav', 'rb'))

liver_model = pickle.load(open('C:/Users/dell/Downloads/liver_model.sav', 'rb'))

diabetes_model = pickle.load(open('C:/Users/dell/Downloads/diabetes_model.sav', 'rb'))
...
if (selected == 'Heart Disease Prediction'):
    ...
    if st.button('**Heart Disease Test Result**'):
        ...
        st.markdown(f"<h3 style='color: {result_color};'><b>{heart_diagnosis}</b></h3>", unsafe_allow_html=True)
...
if (selected == 'Diabetes Prediction'):
    ...
    if st.button('Diabetes Test Result'):
        ...
        st.markdown(f"<h3 style='color: {result_color};'><b>{diab_diagnosis}</b></h3>", unsafe_allow_html=True)
...
if (selected == 'Liver Prediction'):
    ...
    with col1:
        ...
        Aspartate = st.text_input('**Aspartate Aminotransferase value**')
        Albumin = st.text_input('**Albumin**')
    with col2:
        Gender = st.selectbox('**Gender**', ['Male', 'Female'])
        Direct_Bilirubin = st.text_input('**Direct Bilirubin value**')
        Alamine = st.text_input('**Alamine Aminotransferase value**')
        Total_Proteins = st.text_input('**Total Proteins value**')
        Albumin_Globulin_Ratio = st.text_input('**Albumin and Globulin Ratio value**')
    ...
    alkaline_phosphotase = float(Alkaline_Phosphotase) if Alkaline_Phosphotase else None
    alamine = float(Alamine) if Alamine else None
    aspartate = float(Aspartate) if Aspartate else None
    total_proteins = float(Total_Proteins) if Total_Proteins else None
    albumin = float(Albumin) if Albumin else None
    albumin_globulin_ratio = float(Albumin_Globulin_Ratio) if Albumin_Globulin_Ratio else None
    ...
    if st.button('**Liver Test Result**'):
        ...
        if not (age and total_bilirubin and direct_bilirubin and alkaline_phosphotase and alamine and aspartate and total_proteins and albumin and albumin_globulin_ratio):
            st.error('Please fill in all the required fields.')
        else:
            # Pass input values to the prediction model
            liver_prediction = liver_model.predict([[age, gender, total_bilirubin, direct_bilirubin, alkaline_phosphotase, alamine, aspartate, total_proteins, albumin, albumin_globulin_ratio]])
            ...
            st.markdown(f"<h3 style='color: {result_color};'><b>{liver_diagnosis}</b></h3>", unsafe_allow_html=True)
...
if (selected == 'Cancer Prediction'):
    ...
    if st.button('**Cancer Test Result**'):
        ...
        st.markdown(f"<h3 style='color: {result_color};'><b>{cancer_diagnosis}</b></h3>", unsafe_allow_html=True)
...
if (selected == 'Kidney Prediction'):
    ...
    al = float(al) if al else None
    su = float(su) if su else None
    bgr = float(bgr) if bgr else None
    bu = float(bu) if bu else None
    sc = float(sc) if sc else None
    pot = float(pot) if pot else None
    wc = int(wc) if wc else None
    ...
    if st.button('**Kidney Test Result**'):
        # Pass input variables as a list to the predict() function
        Kidney_diagnosis = kidney_model.predict([[age, bp, al, su, bgr, bu, sc, rbc, pc, pcc, ba, pot, wc, htn, dm, cad, pe, ane]])

        if(Kidney_diagnosis[0]==1):
            Kidney_diagnosis = 'The person has a Kidney Problem'
            result_color = "red"
        else:
            Kidney_diagnosis = 'The person does not have a Kidney Problem'
            result_color = "green"

        st.markdown(f"<h3 style='color: {result_color};'><b>{Kidney_diagnosis}</b></h3>", unsafe_allow_html=True)
